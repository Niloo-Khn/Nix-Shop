...
const fallbackPrices = { "cloud-bed": 68, "mouse-toy": 14, "walk-set": 42, "slow-bowl": 24, "groom-brush": 18, "treat-pouch": 22 };
function styledProduct(record) {
...
class StorefrontService {
    constructor() {
        this.accountsApi = new ApiClient(API_ENDPOINTS.accounts);
...
function safelyParseCart(value) {
    try {
        ...
        return parsed.flatMap((item) => {
            ...
            if (typeof candidate.id !== "string" || !Number.isInteger(candidate.quantity) || Number(candidate.quantity) < 1)
                return [];
...
function notFoundPage() { return `${header()}<main id="main" class="not-found"><p class="eyebrow">${t("notFound.eyebrow")}</p><h1>${t("notFound.title")}</h1><p>${t("notFound.body")}</p><a class="button" href="#/">${t("notFound.action")}</a></main>${footer()}`; }
...
function bindActions() {
    ...
    document.querySelector("#checkout-button")?.addEventListener("click", async () => {
        ...
        try {
            ...
            if (message)
                message.textContent = t("checkout.unavailable");
        }
    });
    bindAccountForms();
    bindHelpCenter();
...
function bindAccountForms() {
    document.querySelector("#login-form")?.addEventListener("submit", async (event) => {
        ...
        try {
            const result = await storefront.login({ email: formValue(form, "email"), password: formValue(form, "password") });
            sessionStorage.setItem("nix-shop-access-token", result.accessToken);
            setFormStatus(form, t("account.signedIn"));
            form.reset();
        }
        catch {
            setFormStatus(form, t("account.loginFailed"));
        }
    });
    document.querySelector("#register-form")?.addEventListener("submit", async (event) => {
        ...
        const form = event.currentTarget;
        try {
            const result = await storefront.register({ email: formValue(form, "email"), displayName: formValue(form, "displayName"), password: formValue(form, "password") });
            setFormStatus(form, t("account.welcome", { name: result.displayName }));
            form.reset();
        }
        catch {
            setFormStatus(form, t("account.createFailed"));
        }
    });
}
function bindHelpCenter() {
    const container = document.querySelector("#help-articles");
    if (container)
        storefront.helpArticles().then((articles) => {
            container.replaceChildren(...articles.flatMap((article) => { const content = helpArticleCopy(article.id); if (!content)
                return []; const card = document.createElement("article"); const title = document.createElement("h3"); const body = document.createElement("p"); title.textContent = content.title; body.textContent = content.body; card.append(title, body); return [card]; }));
        }).catch(() => { container.textContent = t("help.unavailable"); });
    document.querySelector("#ticket-form")?.addEventListener("submit", async (event) => {
        event.preventDefault();
        const form = event.currentTarget;
        try {
            const ticket = await storefront.createTicket({ email: formValue(form, "email"), subject: formValue(form, "subject"), message: formValue(form, "message") });
            setFormStatus(form, t("help.received", { id: ticket.id.slice(0, 8) }));
            form.reset();
        }
        catch {
            setFormStatus(form, t("help.sendFailed"));
        }
    });
}
function render() {
