...
function notFoundPage() { return `${header()}<main id="main" class="not-found"><p class="eyebrow">404</p><h1>That page wandered off.</h1><p>Let's get you back to the good stuff.</p><a class="button" href="#/">Go home</a></main>${footer()}`; }
...
function bindAccountForms() {
    document.querySelector("#login-form")?.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            setFormStatus(form, "You are signed in for this browser session.");
            form.reset();
        }
        catch {
            setFormStatus(form, "Sign-in failed. Check your details and make sure the account service is running.");
        }
    });
    document.querySelector("#register-form")?.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            setFormStatus(form, `Welcome, ${result.displayName}. Your account was created.`);
            form.reset();
        }
        catch {
            setFormStatus(form, "Account creation failed. Check the form and make sure the account service is running.");
        }
    });
}
function bindHelpCenter() {
    ...
    document.querySelector("#ticket-form")?.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            setFormStatus(form, `Message received. Ticket ${ticket.id.slice(0, 8)} was created.`);
            form.reset();
        }
        catch {
            setFormStatus(form, "We couldn't send your message. Make sure the help service is running.");
        }
    });
}
function render() {
